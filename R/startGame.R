...
startGame <- function(...){
  if (exists(".gameOn")) {
    ...
    continue <- FALSE
    playerLevel <- NULL
    if (file.exists(file.path(find.package("JCastleOfR"), "CastleOfR_game.RData"))) {
      message("前に来た時のデータがあります。前の続きから始めますか?")
      continue <- menu(c("yes", "no")) == 1
    }
    game <- if (continue) {
      readRDS(file.path(find.package("JCastleOfR"), "CastleOfR_game.RData"))
    } else {
    ...
    addTaskCallback(cb, name = "JCastleOfR")
    invisible()
  }
}